...
class FinishTaskFromTechScreen extends StatefulWidget {
  const FinishTaskFromTechScreen({
    super.key,
    required this.price,
...
class _FinishTaskFromTechScreenState extends State<FinishTaskFromTechScreen> {
  ...
  Future<bool?> _showConfirmDialog() {
    return showDialog<bool>(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text("تأكيد إغلاق الطلب"),
          content: const Text("هل أنت متأكد من إنهاء المهمة وإغلاق الطلب؟"),
          actions: [
            TextButton(
              onPressed: () {
      ...
      listener: (context, state) {
        if (state is ChangeStatusOrderSuccess) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text("تم إغلاق الطلب بنجاح")));
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: SafeArea(
          child: Padding(
            ...
            child: BlocBuilder<ChangeStatusOrderCubit, ChangeStatusOrderState>(
